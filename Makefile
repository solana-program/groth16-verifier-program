...
# CI matrix entry -> manifest directory. `groth16-verify` and `program` are the
# crates a consumer depends on; the converters and the bench program are
# development tools and live under tools/.
path-groth16-convert = tools/convert
path-bench = tools/bench
make-path = $(or $(path-$1),$1)
...
# `make <target>-<package>` for package in program, bench, solana-groth16-verify,
# groth16-convert.
...
# SBF artifacts. `solana-groth16-program` and `groth16-bench` are the crate names, so
# the outputs are target/deploy/solana_groth16_program.so and groth16_bench.so.
build-sbf-%:
...
# Regenerate tools/fixtures/gnark/*.bin (needs Go).
fixtures:
	cd tools/fixtures/gnark/gen && go run .

# Host line coverage (needs `cargo install cargo-llvm-cov` and the
# `llvm-tools-preview` component). `program/src` and `tools/bench/src` only execute
