//! Parses the gnark fixture under `fixtures/gnark/` and checks the converted
//! bytes three ways: compressed and raw encodings agree, arkworks verifies the
//! round-tripped key and proof, and `groth16-verify`'s host path accepts the
...
use {
    ...
    groth16_verify::{Proof, VerifyingKey},
};
...
#[test]
fn groth16_verify_host_path_accepts_the_wire_bytes() {
    let key = gnark::parse_verifying_key(&fixture("vk.bin")).unwrap();
    let proof = gnark::parse_proof(&fixture("proof.bin")).unwrap();
    let inputs = gnark::parse_public_witness(&fixture("public.bin")).unwrap();
    ...
    groth16_verify::verify(&vk, &proof, &flat).unwrap();

    let mut wrong = flat.clone();
    wrong[95] ^= 1;
    assert_eq!(
        groth16_verify::verify(&vk, &proof, &wrong),
        Err(groth16_verify::Groth16Error::ProofInvalid)
    );
    assert_eq!(
        groth16_verify::verify(&vk, &proof, &flat[..64]),
        Err(groth16_verify::Groth16Error::PublicInputCountMismatch)
    );
}
